...
public:
    halow_sync_long_impl(unsigned int sync_length, bool log, bool debug)
        : block("halow_sync_long",
...
{
    return gnuradio::get_initial_sptr(new halow_sync_long_impl(sync_length, log, debug));
}

const std::vector<gr_complex> halow_sync_long_impl::LONG = {
