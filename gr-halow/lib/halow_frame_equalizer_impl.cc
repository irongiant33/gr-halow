...
#include "equalizer/halow_ls.h"
#include "halow_frame_equalizer_impl.h"
#include "utils.h"
...
halow_frame_equalizer::sptr
halow_frame_equalizer::make(Equalizer algo, double freq, double bw, bool log, bool debug)
{
    return gnuradio::get_initial_sptr(
        new halow_frame_equalizer_impl(algo, freq, bw, log, debug));
}


halow_frame_equalizer_impl::halow_frame_equalizer_impl(
    Equalizer algo, double freq, double bw, bool log, bool debug)
    : gr::block("halow_frame_equalizer",
...
void halow_frame_equalizer_impl::set_algorithm(Equalizer algo)
{
    ...
    case STA:
        ...
        break;
    case HALOW_LS:
        dout << "HALOW_LS" << std::endl;
        d_equalizer = new equalizer::halow_ls();
        break;
    default:
...
void halow_frame_equalizer_impl::set_bandwidth(double bw)
{
    gr::thread::scoped_lock lock(d_mutex);
    d_bw = bw;
}

void halow_frame_equalizer_impl::set_frequency(double freq)
{
    gr::thread::scoped_lock lock(d_mutex);
    d_freq = freq;
}

void halow_frame_equalizer_impl::forecast(int noutput_items,
                                    gr_vector_int& ninput_items_required)
{
    ninput_items_required[0] = noutput_items;
}

int halow_frame_equalizer_impl::general_work(int noutput_items,
...
bool halow_frame_equalizer_impl::decode_signal_field(uint8_t* rx_bits)
{
...
void halow_frame_equalizer_impl::deinterleave(uint8_t* rx_bits)
{
    for (int i = 0; i < 48; i++) {
...
bool halow_frame_equalizer_impl::parse_signal(uint8_t* decoded_bits)
{
...
const int halow_frame_equalizer_impl::interleaver_pattern[48] = {
